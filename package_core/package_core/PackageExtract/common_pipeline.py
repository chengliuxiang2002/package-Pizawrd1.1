...
def remove_ctr_gt10_case_sensitive(bottom_ocr_data,tol = 10.0):
    """
    删除 bottom_ocr_data 中满足以下条件的条目（大小写敏感）：
    1) item['ocr_strings'] 中包含子串 'CTR'（必须是大写CTR）
    2) item['key_info'] 里能解析出的数字 > 10

    返回过滤后的新列表（不修改原列表）。
    """
    def extract_numbers_from_key_info(key_info):
        nums = []
        if not key_info:
            return nums
        # key_info 形如 [['12'], ['0.8']] / [['13.5','±','0.1']] 等
        for group in key_info:
            if not group:
                continue
            for x in group:
                try:
                    nums.append(float(str(x).replace("±", "").strip()))
                except Exception:
                    pass
        return nums

    filtered = []
    for item in bottom_ocr_data:
        ocr_str = str(item.get("ocr_strings", ""))
        has_ctr = ("CTR" in ocr_str)  # ✅ 大小写敏感

        nums = extract_numbers_from_key_info(item.get("key_info"))
        max_num = max(nums) if nums else None

        # 命中条件：含CTR 且 数值>tol => 删除
        if has_ctr and max_num is not None and max_num > tol:
            continue
...
def finalize_pairs(L3):
    ...
    #0121新增：从ocr层面筛选有效数据（剔除CTR）

    bottom_ocr_data = remove_ctr_gt10_case_sensitive(bottom_ocr_data, tol = 10)
    side_ocr_data = remove_ctr_gt10_case_sensitive(side_ocr_data,tol = 1.7)
...
def compute_BGA_parameters(L3):
    ...
    Body_x_y_length(bottom_ocr_data)
    print(bottom_ocr_data)
    BGA_parameter_list = _pairs_module.get_BGA_parameter_list(
        top_ocr_data,
        bottom_ocr_data,
        side_ocr_data,
        detailed_ocr_data,
    ...
    BGA_parameter_list = _pairs_module.resort_parameter_list_2(BGA_parameter_list)

    if len(BGA_parameter_list[4]["maybe_data"]) > 1:
        high = _pairs_module.get_QFP_high(BGA_parameter_list[4]["maybe_data"])
        if len(high) > 0:
            BGA_parameter_list[4]["maybe_data"] = high
            BGA_parameter_list[4]["maybe_data_num"] = len(high)

    if (
        len(BGA_parameter_list[5]["maybe_data"]) > 1
        or len(BGA_parameter_list[6]["maybe_data"]) > 1
    ):
        pitch_x, pitch_y = _pairs_module.get_QFP_pitch(
            BGA_parameter_list[5]["maybe_data"],
            body_x,
            body_y,
            nx,
            ny,
        ...
        if len(pitch_x) > 0:
            BGA_parameter_list[5]["maybe_data"] = pitch_x
            BGA_parameter_list[5]["maybe_data_num"] = len(pitch_x)
        if len(pitch_y) > 0:
            BGA_parameter_list[6]["maybe_data"] = pitch_y
            BGA_parameter_list[6]["maybe_data_num"] = len(pitch_y)

    BGA_parameter_list = _pairs_module.resort_parameter_list_2(BGA_parameter_list)

    # run_and_save_resort_log(BGA_parameter_list) # 将参数候选列表保存至txt文件

    return BGA_parameter_list, nx, ny
